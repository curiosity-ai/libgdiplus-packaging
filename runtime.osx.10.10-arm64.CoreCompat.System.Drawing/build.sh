...
#cd "$path/libgdiplus"
#./autogen.sh --prefix="$out" CPPFLAGS="\"-I/usr/X11/include/ -I/opt/homebrew/opt/jpeg/include/  -I/opt/homebrew/opt/jpeg/include/  -I/opt/homebrew/Cellar/libtiff/4.4.0_1/include\"" --without-x11
#make clean
#make
##make check
#make install
...
dylibs=`otool -L "$out/lib/libgdiplus.dylib" | grep "/opt/homebrew" | awk -F' ' '{ print $1 }'`
echo $dylibs
...
dylibs=`otool -L "$out/lib/libcairo.2.dylib" | grep "/opt/homebrew" | awk -F' ' '{ print $1 }'`
...
dylibs=`otool -L "$out/lib/libxcb.1.dylib" | grep "/opt/homebrew" | awk -F' ' '{ print $1 }'`
...
# libxcb has various dependencies, so include those too
dylibs=`otool -L "$out/lib/libglib-2.0.0.dylib" | grep "/opt/homebrew" | awk -F' ' '{ print $1 }'`
...
for f in "$out/lib/"*.dylib; do
   ...
   dylibs=`otool -L "$f" | tail -n +3 | grep "/opt/homebrew" | awk -F' ' '{ print $1 }'`
